...
int MY_OLED_WriteNBytes(uint8_t *buf, uint16_t length)
{
    return HAL_I2C_Mem_Write(g_pHI2COLED, OELD_I2C_ADDR, 0x40, 1, buf, length, OLED_TIMEOUT);
}
...
void OLED_PutChar_Reverse(uint8_t x, uint8_t y, char c)
{
    uint8_t page = y;
    uint8_t col = x * 8;
    if (y > 7 || x > 15)
    ...
    {
        str[i] = ~ascii_font[c][i];
        str[i + 8] = ~ascii_font[c][i + 8];
    }
    // ascii_font[c][0];

    OLED_SetPosition(page, col);
    OLED_WriteNBytes(str, 8);

    OLED_SetPosition(page + 1, col);
    OLED_WriteNBytes(&str[8], 8);
}
...
int OLED_PrintString_Reverse(uint8_t x, uint8_t y, const char *str)
{
    int i = 0;
    while (str[i])
    {
        OLED_PutChar_Reverse(x, y, str[i]);
        x++;
        if (x > 15)
        {
            x = 0;
